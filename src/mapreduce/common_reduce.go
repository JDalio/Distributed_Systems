...
func doReduce(
	...
	for i := 0; i < nMap; i++ {
		go func(nthMap int) {
			...
			if err != nil {
				log.Fatal("Reduce Step1 open file:", err)
			}
			defer file.Close()
			...
			for scanner.Scan() {
				...
				if err := json.Unmarshal(lineByt, &entry); err != nil {
					log.Fatal("Reduce Step1 decode:", err)
				}
				kvChan <- entry
			}
			manageKvChan <- 1
	...
	file, err := os.OpenFile(outFile, os.O_CREATE|os.O_APPEND, 0644)
	if err != nil {
		log.Fatal("Reduce Step3 open result file:", err)
	}
	defer file.Close()
	...
	for _, k := range keys {
		...
		if err != nil {
			log.Fatal("Reduce Step1 write encode result:", err)
		}
	}
}
